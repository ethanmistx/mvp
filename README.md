...
- **健康(生病周期)**:体温快捷记录(部位 / 是否用退烧药)+ 72h 迷你曲线(38°C 中性参考线);用药疗程打卡(药名 / 每日次数 / 疗程天数,今日 N/M 进度),复诊时摘要即病程记录——通用设计,发烧、抗生素疗程、疫苗后观察都适用,**不做任何"发烧/异常"判断**
- **留存设计**:首页显示「出生第 N 天 · X 个月 Y 天」与「已连续记录 N 天」(当天未记录不打断连续,从昨天起算)
- **主题**:深色(默认)/ 浅色 / 跟随系统三档切换;全部语义色走 CSS 变量,原生控件与图表同步换肤
- **质感**:iOS 风格毛玻璃材质(底部导航 / 弹层 / 对话框 / Toast,`backdrop-filter` 不可用或系统开启"降低透明度"时自动退回不透明)
...
数据模型:`BabyProfile` / `Feed` / `Sleep`(`end=null` 表示进行中)/ `Growth` / `Diaper` / `Temperature` / `MedCourse` + `MedDose`,字段见 `src/types.ts`。备份格式当前为 `schemaVersion: 2`,v1 旧备份导入时自动迁移(健康集合补空)。
